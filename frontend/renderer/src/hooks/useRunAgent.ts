...
export const useRunAgent = () => {
  ...
  const { ensurePermissions, getEnsurePermissions } = usePermissions(false);

  const run = async (
    value: string,
    history: ConversationWithSoM[],
    callback: () => void = () => {},
    options: { autoSkillSummaryEnabled?: boolean; operator?: Operator } = {},
  ) => {
    const operator = options.operator || settings.operator;
    ...
    if (operator === Operator.LocalBrowser || operator === Operator.LocalComputer) {
      const nextPermissions =
        (await getEnsurePermissions()) || ensurePermissions || {};
      if (!(nextPermissions.accessibility && nextPermissions.screenCapture)) {
        const permissionsText = [
          !nextPermissions.screenCapture ? 'screenCapture' : '',
          !nextPermissions.accessibility ? 'Accessibility' : '',
        ]
          .filter(Boolean)
          .join(' and ');
    ...
    await api.runAgent({
      autoSkillSummaryEnabled: Boolean(options.autoSkillSummaryEnabled),
    });
