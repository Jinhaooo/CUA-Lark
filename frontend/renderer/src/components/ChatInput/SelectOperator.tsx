...
export const SelectOperator = () => {
  ...
  useEffect(() => {
    if (settings.operator === Operator.LocalBrowser && !browserAvailable) {
      ...
      toast.info(`已自动切换为${COMPUTER_OPERATOR}模式`, {
        description: '浏览器模式不可用',
      });
    }
  ...
  const handleRetryBrowserCheck = async () => {
    try {
      ...
      if (available) {
        toast.success('已检测到浏览器', {
          description: '现在可以使用浏览器模式了。',
        });
        setTooltipOpen(false);
      } else {
        toast.error('未检测到浏览器', {
          description: '请安装 Chrome 后再重试。',
        });
      }
    } catch (error) {
      toast.error('检查浏览器可用性失败');
    } finally {
      setIsRetrying(false);
    }
  };
