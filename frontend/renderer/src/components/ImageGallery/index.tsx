...
const ImageGallery: React.FC<ImageGalleryProps> = ({
      ...
      <>
        {currentEntry.actions.map((action, idx) => {
          ...
          return (
            <div
              key={idx}
              className="flex min-w-0 flex-shrink-0 items-center gap-1.5 py-1"
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mr-2 max-w-full truncate text-[11px] leading-4 text-muted-foreground">
                  <span className="font-medium text-primary/70">
                    {action.type}
                  </span>
                  {action.input && (
                    <span className="max-w-full truncate text-primary/70">
                      {action.input}
                    </span>
                  )}
                  {action.cost && (
  ...
  return (
    <div className="h-full flex flex-col rounded-xl bg-white py-2">
      <div className="mb-2 flex min-h-[34px] gap-2 overflow-x-auto">
        {renderActions()}
      </div>
